...
namespace TownOfUs.EngineerMod
{
  ...
  public class ColourChange
  {
    private static void UpdateMeeting(OOCJALPKPEP __instance)
    {
      ...
      using (IEnumerator<HDJGDMFCHDN> enumerator = ((Il2CppArrayBase<HDJGDMFCHDN>) __instance.get_HBDFFAHBIGI()).GetEnumerator())
      {
        while (((IEnumerator) enumerator).MoveNext())
        {
          ...
          if ((Object.op_Inequality((Object) Utils.Engineer, (Object) null) && current.get_NameText().get_Text() == Utils.Engineer.get_nameText().get_Text()) & FFGALNAPKCD.get_LocalPlayer().isEngineer())
            current.get_NameText().set_Color(new Color(1f, 0.7f, 0.0f, 1f));
        }
      }
    }
    ...
    public static void Postfix(PIEFJFEOGOL __instance)
    {
      ...
      if (!(FFGALNAPKCD.get_AllPlayerControls().get_Count() > 1 & FFGALNAPKCD.get_LocalPlayer().isEngineer()))
        return;
      FFGALNAPKCD.get_LocalPlayer().get_nameText().set_Color(new Color(1f, 0.7f, 0.0f, 1f));
    }
  }
}

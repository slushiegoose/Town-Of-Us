...
// Type: TownOfUs.MayorMod.ColourChange
...
namespace TownOfUs.MayorMod
{
  ...
  public class ColourChange
  {
    private static void UpdateMeeting(OOCJALPKPEP __instance)
    {
      using (IEnumerator<HDJGDMFCHDN> enumerator = ((Il2CppArrayBase<HDJGDMFCHDN>) __instance.get_HBDFFAHBIGI()).GetEnumerator())
      {
        while (((IEnumerator) enumerator).MoveNext())
        {
          ...
          if ((Object.op_Inequality((Object) Utils.Mayor, (Object) null) && current.get_NameText().get_Text() == Utils.Mayor.get_nameText().get_Text()) & FFGALNAPKCD.get_LocalPlayer().isMayor())
            current.get_NameText().set_Color(new Color(0.44f, 0.31f, 0.66f, 1f));
        }
      }
    }
    ...
    public static void Postfix(PIEFJFEOGOL __instance)
    {
      ...
      if (!(FFGALNAPKCD.get_AllPlayerControls().get_Count() > 1 & FFGALNAPKCD.get_LocalPlayer().isMayor()))
        return;
      FFGALNAPKCD.get_LocalPlayer().get_nameText().set_Color(new Color(0.44f, 0.31f, 0.66f, 1f));
    }
  }
}

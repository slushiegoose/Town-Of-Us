...
// Type: TownOfUs.TimeMasterMod.ColourChange
...
namespace TownOfUs.TimeMasterMod
{
  ...
  public class ColourChange
  {
    private static void UpdateMeeting(OOCJALPKPEP __instance)
    {
      using (IEnumerator<HDJGDMFCHDN> enumerator = ((Il2CppArrayBase<HDJGDMFCHDN>) __instance.get_HBDFFAHBIGI()).GetEnumerator())
      {
        while (((IEnumerator) enumerator).MoveNext())
        {
          ...
          if ((Object.op_Inequality((Object) Utils.TimeMaster, (Object) null) && current.get_NameText().get_Text() == Utils.TimeMaster.get_nameText().get_Text()) & FFGALNAPKCD.get_LocalPlayer().isTimeMaster())
            current.get_NameText().set_Color(new Color(0.0f, 0.0f, 1f, 1f));
        }
      }
    }
    ...
    public static void Postfix(PIEFJFEOGOL __instance)
    {
      ...
      if (!(FFGALNAPKCD.get_AllPlayerControls().get_Count() > 1 & FFGALNAPKCD.get_LocalPlayer().isTimeMaster()))
        return;
      FFGALNAPKCD.get_LocalPlayer().get_nameText().set_Color(new Color(0.0f, 0.0f, 1f, 1f));
    }
  }
}

...
// Type: TownOfUs.SwapperMod.ColourChange
...
namespace TownOfUs.SwapperMod
{
  ...
  public class ColourChange
  {
    private static void UpdateMeeting(OOCJALPKPEP __instance)
    {
      using (IEnumerator<HDJGDMFCHDN> enumerator = ((Il2CppArrayBase<HDJGDMFCHDN>) __instance.get_HBDFFAHBIGI()).GetEnumerator())
      {
        while (((IEnumerator) enumerator).MoveNext())
        {
          ...
          if ((Object.op_Inequality((Object) Utils.Swapper, (Object) null) && current.get_NameText().get_Text() == Utils.Swapper.get_nameText().get_Text()) & FFGALNAPKCD.get_LocalPlayer().isSwapper())
            current.get_NameText().set_Color(new Color(0.4f, 0.9f, 0.4f, 1f));
        }
      }
    }
    ...
    public static void Postfix(PIEFJFEOGOL __instance)
    {
      ...
      if (!(FFGALNAPKCD.get_AllPlayerControls().get_Count() > 1 & FFGALNAPKCD.get_LocalPlayer().isSwapper()))
        return;
      FFGALNAPKCD.get_LocalPlayer().get_nameText().set_Color(new Color(0.4f, 0.9f, 0.4f, 1f));
    }
  }
}
